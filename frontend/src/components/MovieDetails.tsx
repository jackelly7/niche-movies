...
const MovieDetails: React.FC<MovieDetailsProps> = ({
	...
	const handleRate = async (rating: number) => {
		...
		try {
			const response = await fetch(
				"https://niche-movies-backend-1-b8f2anendma6dhbd.eastus-01.azurewebsites.net/rate",
				{
					method: "POST",
	...
	useEffect(() => {
		const fetchRating = async () => {
			...
			try {
				const response = await fetch(
					`https://niche-movies-backend-1-b8f2anendma6dhbd.eastus-01.azurewebsites.net/GetRating?userId=${userId}&showId=${movie.showId}`
				);
				if (response.ok) {
