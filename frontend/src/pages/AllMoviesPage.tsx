...
const genreMap: Record<string, string> = {
  Action: "Action",
  Adventure: "Adventure",
  Anime_Series_International_TV_Shows: "Anime Series / International TV Shows",
...
const AllMoviesPage = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      setLoading(true);
      try {
        const res = await axios.get("https://localhost:4000/AllMovies", {
          params: {
            page,
            limit: 50,
            genre: genreKey,
  ...
  useEffect(() => {
    const fetchPosters = async () => {
      try {
        const postersRes = await axios.get("https://localhost:4000/poster");
  ...
  useEffect(() => {
    ...
    setMovies((prev) =>
      prev.map((movie) => ({
        ...movie,
        posterUrl:
          posters.length > 0 ? matchPoster(movie.title, posters) : undefined,
      }))
    );
  }, [posters]);
  ...
  const filteredMovies = useMemo(() => {
    return movies.filter((movie) => {
      if (searchQuery.trim() !== "") {
        ...
        const titleMatch = (movie.title ?? "").toLowerCase().includes(lower);
    ...
    <div className="pt-20 container mx-auto px-4 pb-10">
      ...
      {loading && movies.length === 0 ? (
        <div className="col-span-full flex justify-center items-center py-20">
          ...
          <span className="ml-4 text-gray-600 text-lg">Loading movies...</span>
        </div>
      ) : (
...
const MovieCard = ({
  ...
  return (
    <div ref={ref} className="relative group cursor-pointer" onClick={onSelect}>
